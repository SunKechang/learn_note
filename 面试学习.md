...
1. InnoDB 会根据以下两个条件检查每行记录： 
   ...
   b ．行的删除版本要么未定义．要么大于当前事务版本号。这可以确保事务读取到的行，在事务开始之前未被侧除。只有符合上违两个条件的记录，才能返回作为查询结果。 
...
1. InnoDB 为删除的每一行保存当前系统版本号作为行侧除标识。 
